...
# 구조 신호 정규식 (tools.py 반환 포맷에 대응 — 안내문 문구 변화에 덜 취약하게
# 포맷 자체의 구조를 본다).
#   search_logs 파일 헤더:  "network.log: 2건 — '대표줄'..."  → 파일명 + 매치 건수
_SEARCH_LOGS_LINE = re.compile(r"^(.+?): (\d+)건 — ")
#   read_file(keyword)/read_source 줄:  "42: 내용"  → 줄번호 prefix
_NUMBERED_LINE = re.compile(r"^\d+: ")
#   read_file(keyword) 블록 상한 초과 표기:  "...외 3블록 (...)"
_CONTEXT_OVERFLOW = re.compile(r"\.\.\.외 (\d+)블록")
#   search_code 매치 줄:  "경로:42: 내용"  (overflow 표기 "...외 N건" 은 미포함)
_SEARCH_CODE_LINE = re.compile(r":\d+: ")


def _tool_result_summary(name: str, raw: str, is_error: bool) -> str:
    """도구 실행 결과를 한 줄 요약 로그 문구로 변환(raw=마스킹 전 기준, 원문 내용 미노출).

    의미 기반 요약(사용자 확정): 전체 읽기(read_file 무키워드·read_source) 외에는
    글자수 대신 '무엇을 얼마나 찾았는지'를 노출한다. 판정은 안내문 문자열이 아니라
    tools.py 반환 포맷의 **구조 신호**(': N건 —' 헤더, '줄번호:' prefix, '---' 블록
    구분)를 우선으로 하고, 무매치 안내문만 문구 패턴으로 보조 판정한다.

    - 실패(에러 문자열): "└ 결과: 실패 — <에러 첫 60자>"
    - search_logs: 매치 없음 → "매치 없음". 매치 → "N개 파일 매치 (상위3 · 표기, 외 M개)".
    - read_file(keyword 有): 무매치 → "매치 없음". 매치 → "매치 N건 (±2줄 포함 M줄)".
      keyword 無(전체 읽기)면 구조 신호가 없어 "N줄 / N자 반환" 으로 떨어진다.
    - read_source(전체 읽기): "N줄 / N자 반환" (글자수 유의미하게 유지).
    - search_code: 매치 없음 → "매치 없음". 매치 → "매치 N줄".
    ...
    if name == "read_source":
        # 전체 읽기 — 글자수가 유의미하므로 줄수/글자수 유지.
        return f"└ 결과: {len(raw.splitlines())}줄 / {len(raw)}자 반환"
    if name == "read_file":
        return _summary_read_file(raw)
    if name == "search_logs":
        return _summary_search_logs(raw)
    if name == "search_code":
        # "경로:줄번호: 내용" 목록. 매치 줄만 세고(overflow 표기 "...외 N건" 제외),
        # 무매치 안내문("...가 없습니다.")은 매치 줄이 0이라 자연히 "매치 없음" 으로 떨어진다.
        matches = sum(1 for ln in raw.splitlines() if _SEARCH_CODE_LINE.search(ln))
        if matches:
...
def _summary_search_logs(raw: str) -> str:
    """search_logs 요약: 파일별 '이름: N건 —' 헤더를 파싱해 매치 파일 수·상위 3개를 노출.

    무매치 안내문("'kw' 를 포함하는 로그 파일이 없습니다.")은 헤더가 0개라 "매치 없음".
    파일이 많으면 매치 건수 상위 3개만 보이고 나머지는 "외 N개".
    """
    files = []  # (파일명, 매치건수)
    for ln in raw.splitlines():
        m = _SEARCH_LOGS_LINE.match(ln)
        if m:
            files.append((m.group(1), int(m.group(2))))
    if not files:
        return "└ 결과: 매치 없음"
    top = sorted(files, key=lambda x: x[1], reverse=True)[:3]
    parts = " · ".join(f"{name} {cnt}건" for name, cnt in top)
    extra = len(files) - len(top)
    suffix = f" 외 {extra}개" if extra > 0 else ""
    return f"└ 결과: {len(files)}개 파일 매치 ({parts}{suffix})"


def _summary_read_file(raw: str) -> str:
    """read_file 요약: keyword 조회('줄번호:' + '---' 블록 구조)면 의미 요약, 아니면 전체 읽기.

    - keyword 매치: "매치 N건 (±2줄 포함 M줄)" — N=매치 블록(±2줄 컨텍스트로 병합된 구간)
      수, M=반환된 번호줄 총수. 상한 초과 표기("...외 N블록")가 있으면 그 수도 N에 더한다.
    - keyword 무매치: "'kw' 를 포함하는 줄이 없습니다." → 번호줄이 0이므로 "매치 없음".
    - keyword 無(전체 읽기): 번호줄 구조가 없어 "N줄 / N자 반환" 으로 떨어진다.
    """
    blocks = 0
    numbered = 0
    overflow_blocks = 0
    for seg in raw.split("\n---\n"):
        seg_numbered = sum(1 for ln in seg.splitlines() if _NUMBERED_LINE.match(ln))
        if seg_numbered:
            blocks += 1
            numbered += seg_numbered
        mo = _CONTEXT_OVERFLOW.search(seg)
        if mo:
            overflow_blocks += int(mo.group(1))
    if numbered:
        return f"└ 결과: 매치 {blocks + overflow_blocks}건 (±2줄 포함 {numbered}줄)"
    if "를 포함하는 줄이 없습니다" in raw:
        return "└ 결과: 매치 없음"
    # keyword 無 전체 읽기 — 글자수 유의미.
    return f"└ 결과: {len(raw.splitlines())}줄 / {len(raw)}자 반환"
