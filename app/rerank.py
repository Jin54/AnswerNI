...
- "tfidf"(기본): 위의 TF-IDF 코사인. 외부 의존성·네트워크 없음.
...
- "hybrid": 두 점수를 RRF(Reciprocal Rank Fusion)로 결합. 점수 스케일이 다른 두
  방식을 min-max 정규화 없이 순위만으로 섞어 이상치·스케일 차이에 강건하다.
...
def _mode() -> str:
    """리랭킹 모드 — env RERANK_MODE 를 호출 시점에 평가. 미지의 값은 tfidf 로 방어."""
    m = (os.environ.get("RERANK_MODE") or "tfidf").strip().lower()
    return m if m in _MODES else "tfidf"
...
def rerank(query: str, issues: "list[dict]", top_k: int,
    ...
    - 점수 산출 방식은 env RERANK_MODE(tfidf|embedding|hybrid, 기본 tfidf)로 선택하며
